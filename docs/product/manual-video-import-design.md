...
状态：已确认（2026-07-16，HTML 原型已验收；当前实现已获提交授权，正式发布前仍需真实 Finder 验证）
...
- 将现有单根目录替换模型改为统一视频集合。
- 文件夹承担批量发现、持续访问授权和已有视频状态维护；直接拖入的视频使用文件级授权。
...
```mermaid
flowchart LR
    MENU["菜单选择文件夹"] --> LC["LibraryAccessCoordinator"]
...
- 菜单入口运行 `NSOpenPanel` 并选择一个文件夹；Finder 拖入入口接收一个或多个视频、一个或多个文件夹或两者组合。
- 两类入口生成统一导入请求，共用授权、发现、判重、入库、刷新、取消和结果反馈流程。
- 拖入批次先按标准化 URL 去重，并由父文件夹覆盖内部文件和子文件夹；跨批次重叠范围继续依靠视频全局身份避免重复记录。
...
- 菜单选择和直接拖入的文件夹保存文件夹级 Bookmark；直接拖入的视频保存文件级 Bookmark。
...
1. 用户点击“导入视频…”并选择文件夹，或从 Finder 拖入视频、文件夹及其组合。
2. 拖入入口将输入分类、去重并折叠父子重叠来源；菜单入口直接生成单文件夹导入请求。
...
- 空状态说明从菜单栏选择“文件 > 导入视频…”、使用 `⇧⌘I` 或直接拖入视频与文件夹，窗口内不放置导入按钮。
