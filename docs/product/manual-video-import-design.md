...
- 将现有单根目录替换模型改为统一视频集合。
- 文件夹只承担手动导入入口、持续访问授权和已有视频状态维护。
- 在整个 Tag Manager 范围内识别同一个系统文件资源。
- 新视频始终由用户手动导入。
...
```mermaid
flowchart LR
    UI["AppKit 导入入口"] --> LC["LibraryAccessCoordinator"]
    LC --> LP["LibraryPicker"]
...
- 运行 `NSOpenPanel`，一次选择一个文件夹。
- 按标准化根 URL 复用当前进程内已有的授权来源；其他重叠范围依靠视频全局身份避免重复记录。
- 持有当前进程内全部 Security-Scoped Resource 和对应 FSEventsWatcher。
- 启动手动导入并提供取消能力。
- 向界面发送新增、已存在和失败数量。
- 在 MainActor 上协调状态，把目录枚举提交给 `.utility` detached task，把数据库工作提交给 DatabaseStore actor。
...
- MVP 尚未实现父子授权折叠、引用计数短租约或来源管理界面。

### VideoFileDiscovery 与 DatabaseStore

- 使用 `.utility` 优先级递归枚举用户本次选择的目录。
...
- 每个授权来源持有一个 FSEventsWatcher；Watcher 当前只发送“来源发生变化”信号。
...
1. 用户点击“导入视频…”并选择文件夹。
2. LibraryAccessCoordinator 创建或复用授权来源并持有访问对象。
3. VideoFileDiscovery 在 `.utility` detached task 中递归发现受支持扩展名。
4. 每批文件按全局身份分为新增、已存在和失败。
5. 新增视频执行一次 Finder 标签导入，并进入媒体信息与缩略图队列。
6. 已存在视频只补充或更新位置及基础文件属性。
7. 本次没有发现的既有视频保持原状。
8. 完成后显示轻量结果反馈；取消或失败不会回滚已经完整提交的批次。
...
`FSEventsWatcher` 当前按来源独立创建 Stream，并只向 Coordinator 返回变化信号；事件路径、flags 和事件 ID 没有进入运行时状态。
...
- 枚举和文件属性读取运行在 `.utility` detached task，协调状态运行在 MainActor。
...
- 每次文件操作通过 `video_locations` 找到当前可用来源和相对路径。
...
- Purpose Statement：让用户从多个本地文件夹持续扩充统一视频集合，同时清楚区分手动导入和后台静默维护。原型重点验证导入入口、过程反馈、重复导入结果和取消后的状态连续性。
...
- “文件”菜单将“选择视频资料库…”改为“导入视频…”，快捷键使用 `⇧⌘I`。
- Toolbar 不放置导入按钮，保留浏览、搜索、缩略图尺寸和窗格控制。
- 移除“重新扫描资料库”；用户再次执行导入即可增量发现该文件夹中的新视频。
- 空状态说明从菜单栏选择“文件 > 导入视频…”或使用 `⇧⌘I`，窗口内不放置导入按钮。
- 导入期间显示可取消的轻量进度状态，不阻塞已有视频浏览和标签操作。
- 导入结束显示新增、已存在和失败数量，正常状态维护不提供可见进度。
- 文件确认删除后直接从当前网格消失；移动成功后保持选择并更新路径。
- “无法访问”筛选从侧边栏移除。

这属于新的关键交互流程。正式 AppKit 实现前先制作可操作 HTML 原型，覆盖空资料库、导入中、导入结果、重复导入和取消状态，并取得用户确认。
...
- 执行 Debug build 和测试套件。
- 使用真实 A、B、父子目录、重叠目录、外置卷和 Finder 标签样本验收。
- 在大量视频和持续滚动时验证导入、移动、删除和 UI 响应。
- 验证 App 关闭期间发生的移动和删除能够在下次启动后静默协调。
- 用户确认 HTML 原型后实现 AppKit 交互；最终窗口行为仍由用户手动验收。
