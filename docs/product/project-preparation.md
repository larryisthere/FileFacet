...
- 独立 Store/Service 持有资料库、筛选条件、扫描进度和数据库状态
...
- Security-Scoped Bookmark：保存资料库目录长期访问权
- FSEvents：监听目录变化，触发增量协调或受控重扫
...
单独保存 Finder 标签映射，保证用户移动、重命名或合并导入标签以后，后续扫描仍不会按旧名称重复创建标签。仅在 `tags.source` 中记录 `finder` 无法覆盖这一场景。
...
扫描期间更新 `videos.last_seen_scan_id`。一次扫描完整结束后，再把未出现的记录标记为无法访问。扫描中断时不执行缺失标记，可避免权限异常或应用退出导致大量误判。

### 关键约束与索引

- `video_tags(video_id, tag_id)` 唯一约束
- `tags(parent_id, sort_order)` 索引
- `videos(library_id, relative_path)` 唯一约束
- 文件身份字段索引
...
FSEvents 提供“目录发生了变化”的信号，数据库扫描结果继续作为权威状态。事件应先合并和防抖，再执行目标目录协调；出现事件丢失、根目录变化或大量事件时执行完整重扫。应用启动和重新获得资料库权限时也执行协调扫描。

重命名或移动的关联顺序建议：

1. 同一资料库中匹配卷标识符与文件资源标识符。
2. 匹配成功后更新相对路径并保留应用标签。
3. 匹配失败时按新文件写入，旧记录在完整扫描完成后标记为无法访问。
...
- 首次扫描创建“Finder 标签”根节点。
- 使用标准化外部键去重，同时保留用户可见原名。
- 导入映射独立保存，支持标签被重命名、移动或合并后的幂等扫描。
...
锁定动作必须先覆盖主窗口敏感内容，再触发或等待身份验证。窗口失去活动状态、会话锁定、屏幕睡眠时立即安装隐私遮罩；恢复后按策略要求身份验证。遮罩由 AppKit 放在窗口内容最上层，内容为空白或中性占位，不包含文件名、标签或缩略图。

“永不”建议只影响闲置自动锁定；Mac 睡眠和系统锁屏后仍强制锁定。这个定义更符合默认保护隐私的产品原则。
...
- “最近新增”建议定义为首次进入应用索引的时间，避免文件原始创建时间被复制或导出工具改写后造成理解偏差。
...
- 身份验证和隐私遮罩
- 资料库选择与 Security-Scoped Bookmark
- SQLite 基础表和扫描事务
- 快速文件发现、已有索引即时展示、手动重扫
...
6. “最近新增”按首次进入索引的时间计算，默认 30 天。
7. 应用切换时隐私遮罩立即生效；睡眠和系统锁屏后强制重新验证。
